...
def training(dataset, opt, pipe, 
             ...
             coarse_mesh_path, checkpoint, debug_from):
    first_iter = 0
    ...
    if opt.s2_load_ckpt is None:
        assert coarse_mesh_path, "stage-2 need a watertight mesh."
        ggeo.create_from_mesh(opt, coarse_mesh_path, dataset.model_path)
    else:
        (model_params, _) = torch.load(opt.s2_load_ckpt)
        ggeo.load_full(opt, model_params)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        gs_info = ggeo.renew_gaussian(train_mesh=True)
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations):
                ...
                if opt.full_state:
                    torch.save((ggeo.capture(gs_info, True), iteration), scene.model_path + "/chkpnt" + str(iteration) + "_full.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_checkpoint", type=str, default=None)  # load from stage-1
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), 
             ...
             args.coarse_mesh_path, args.start_checkpoint, args.debug_from)
