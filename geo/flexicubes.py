...
class FlexiCubes:
    ...
    def construct_voxel_grid_v2(self, res, xyz_min=None, xyz_max=None):
        if isinstance(res, int):
            res = [res, res, res]
        if xyz_min is None:
            xyz_min = [-0.5, -0.5, -0.5]
        ...
        tmp = torch.ones(verts.shape[0], dtype=torch.bool, device=self.device)

        cube_base_ijk = torch.nonzero(tmp.view(res_plus.tolist()))
        ...
        return verts, cubes_flatten_i
    
    def construct_voxel_grid_occ(self, res, occ):
        base_cube_f = torch.arange(8).to(self.device)
        if isinstance(res, int):
            res = (res, res, res)
        assert occ.shape == res
        voxel_grid_template = occ  # torch.ones(res, device=self.device)
